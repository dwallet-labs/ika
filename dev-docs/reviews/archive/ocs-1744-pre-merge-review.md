...
- **STATUS: ✅ fixed** (was recorded 2026-06-26 as ◐ PARTIAL with "docs fix outstanding"; re-verified 2026-07-25 and the docs have since caught up). The currency gate is **live** (built for every mirrored/peer-only node, read-blocking on all three paths), and both lagging docs now say so: `dev-docs/specs/ocs-verified-sui-reads.md` presents the gate as LIVE on mirrored/peer-only nodes, and the `ocs_currency.rs` module doc states "it is live". This row now agrees with the status table above.
...
### `network-mirror-2` — Relay client accepts up to a 1 GB response frame from an untrusted serving peer — **FIXED**
- **Severity: LOW (verified real).** Not blocking.
- **STATUS: ✅ FIXED.** Consumer-side length bounds added: `verified_dynamic_fields_page` rejects `resp.entries.len()` over `min(page_size, MAX_VERIFIED_PAGE_ENTRIES=1024)` *before* `Vec::with_capacity` + the O(entries) verify loop (`ReaderError::OverlongPage`); `pump_changesets` rejects `page.len() > page_limit` before the per-entry BLS verify. The batch path was already bounded by `results.len() != ids.len()` (`ocs-verifier-core-1`). Fault-injected unit test confirms the bound (`an_overlong_dynamic_fields_page_is_rejected`).
...
- **STATUS: ✅ FIXED.** The run loop applies exponential backoff (`next_pump_backoff`: poll-interval → 5 s cap — kept short vs the read path's 30 s because the pump is a live MPC-session feeder that suppresses delivery during a backoff) and escalates `advance()`-failure logging warn→error after 5 consecutive failures, after which the grown backoff throttles the rate to ~1 line/5 s instead of ~20/s. Resets + info-logs on recovery. Backoff growth is a pure, unit-tested helper.
