...
Why not rename the legacy names: a global rename breaks every
operator's dashboards/alerts in one shot, and a large share of the
un-prefixed names are Sui-fork-inherited and match upstream Sui's
(operators reuse Sui dashboards; renaming diverges from upstream
forever). The ratchet gets consistency where it's free — going forward.
