...
```
request_presign() or request_global_presign()
        │
        ▼
UnverifiedPresignCap  (stored, waiting for network)
        │
        │  Network completes presign
        ▼
verify_presign_cap()
        │
        ▼
VerifiedPresignCap  (consumed during signing)
```
...
request_future_sign()
        │
        ▼
UnverifiedPartialUserSignatureCap  (stored with request)
        │
...
verify_partial_user_signature_cap()
        │
        ▼
VerifiedPartialUserSignatureCap  (used to complete signing)
```
...
┌─────────────────────────────────────────────────────────────────┐
│                        DKG                                      │
│  request_dwallet_dkg() → DWalletCap (store permanently)        │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                      Presigning                                 │
│  request_presign() → UnverifiedPresignCap (store in pool)      │
│  verify_presign_cap() → VerifiedPresignCap (use once)          │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                       Signing                                   │
│  approve_message() → MessageApproval (use immediately)         │
│  request_sign() consumes VerifiedPresignCap + MessageApproval  │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    Future Signing                               │
│  request_future_sign() → UnverifiedPartialUserSignatureCap     │
│  verify_partial_user_signature_cap() → VerifiedPartial...Cap   │
│  request_sign_with_partial_user_signature() completes signing  │
└─────────────────────────────────────────────────────────────────┘
```
