...
```typescript
import { Curve, UserShareEncryptionKeys, type HashVersion } from '@ika.xyz/sdk';
...
// Create UserShareEncryptionKeys from the seed (V2 hash by default)
...
console.log('Hash version:', userShareKeys.hashVersion); // 2
```
...
`fromRootSeedKey` accepts an optional third parameter `version` (`HashVersion`), which defaults to `2`.

- **V2 (default):** Uses the correct numeric curve byte in the key derivation hash (`fromCurveToNumber(curve)`).
- **V1 (legacy):** Always uses `0` as the curve byte, regardless of the actual curve. This was a bug where the `Curve` string enum was passed directly to `Uint8Array.from()`, which coerced it to `NaN` then `0`.

<Warning title="Migration Note">
If you registered encryption keys **before this fix** with a **non-SECP256K1** curve (ED25519, SECP256R1, or RISTRETTO), you must pass `1` as the third argument to reproduce your existing keys:

```typescript
// Reproduce legacy keys for a previously registered ED25519 encryption key
const legacyKeys = await UserShareEncryptionKeys.fromRootSeedKey(seed, Curve.ED25519, 1);
```

SECP256K1 keys are unaffected because its curve number is `0`, which matches the buggy V1 behavior.
</Warning>

Serialized keys (`toShareEncryptionKeysBytes` / `fromShareEncryptionKeysBytes`) automatically track the hash version via V1/V2 BCS variants, so deserialized keys always use the correct hash version.
