...
Remove the field (a restart is not required for correctness — the field is simply no longer read) and delete the old seed file. The same state is on the metric `ika_dwallet_mpc_seed_identity_state{state="rotation_complete"}`.
