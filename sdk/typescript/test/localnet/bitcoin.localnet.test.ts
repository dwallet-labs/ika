...
describe('bitcoin localnet — destination + publisher', () => {
	it(
		'spends a P2WPKH UTXO via the plugin (sign + broadcast + confirm)',
		async (test) => {
	...
	it(
		'spends a P2TR script-path UTXO via the plugin',
		async (test) => {
			if (!ready) return test.skip();
			const fixture = makeFixture();
			const publicOutput = fixture.secp256k1.publicKey;
			...
			const { buildP2trScriptPath } = await import(
