...
impl AuthorityPerEpochStoreTrait for AuthorityPerEpochStore {
    ...
    fn next_dwallet_mpc_message(&self, last_consensus_round: Option<Round>) -> IkaResult<Option<(Round, Vec<DWalletMPCMessage>)>> {
        self.tables()
            .map_err(|e| e.into())
            .and_then(|tables| tables.next_dwallet_mpc_message(last_consensus_round))
    }

    fn next_dwallet_mpc_output(&self, last_consensus_round: Option<Round>) -> IkaResult<Option<(Round, Vec<DWalletMPCOutput>)>> {
        self.tables()
            .map_err(|e| e.into())
            .and_then(|tables| tables.next_dwallet_mpc_output(last_consensus_round))
    }

    fn next_verified_dwallet_checkpoint_message(&self, last_consensus_round: Option<Round>) -> IkaResult<Option<(Round, Vec<DWalletCheckpointMessageKind>)>> {
        self.tables()
            .map_err(|e| e.into())
