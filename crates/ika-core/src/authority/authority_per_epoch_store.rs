...
pub trait AuthorityPerEpochStoreTrait: Sync + Send + 'static {
    ...
    /// Returns the session identifier and presign bytes, or None if the pool is empty.
    fn pop_presign(
        &self,
        signature_algorithm: DWalletSignatureAlgorithm,
...
pub struct AuthorityEpochTables {
    ...
    internal_presign_pool_ecdsa_secp256k1: DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)>,
    #[default_options_override_fn = "internal_presign_pool_table_default_config"]
    internal_presign_pool_ecdsa_secp256r1: DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)>,
    #[default_options_override_fn = "internal_presign_pool_table_default_config"]
    internal_presign_pool_eddsa: DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)>,
    #[default_options_override_fn = "internal_presign_pool_table_default_config"]
    internal_presign_pool_taproot: DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)>,
    #[default_options_override_fn = "internal_presign_pool_table_default_config"]
    internal_presign_pool_schnorrkel_substrate: DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)>,
...
impl AuthorityEpochTables {
    ...
    fn presign_pool_table(
        &self,
        signature_algorithm: DWalletSignatureAlgorithm,
    ) -> &DBMap<u64, (SessionIdentifier, Vec<Vec<u8>>)> {
    ...
    pub fn insert_presigns(
        ...
        table.insert(&session_sequence_number, &(session_identifier, presigns))?;
    ...
    pub fn pop_presign(
        ...
        let (session_seq, (session_identifier, mut presigns)) = entry_result?;
        ...
        if presigns.is_empty() {
            ...
            table.insert(&session_seq, &(session_identifier, presigns))?;
        }
