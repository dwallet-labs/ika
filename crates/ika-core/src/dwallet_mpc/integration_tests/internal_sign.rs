...
async fn test_internal_sign_flow() {
    ...
    // Get the internal signing configuration from protocol config
    ...
    // TODO: don't mock, wait for actual presign sessions to finish and populate the pool
    // Pre-populate the presign pool with a mock presign for the internal signing algorithm
    ...
    // TODO: make this fully integrated - we should write a request to the channel to sign on some message, then verify the signature coming from the outgoing channel.
    // TODO: We should use actual values - wait for network dkg and internal presign pool to be populated correctly, and then do internal sign.
...
fn test_internal_sign_dkg_session_id_determinism() {
    ...
    // TODO: further manipulation and edge cases are required, e.g. changing consenous rounds and number of internal sessions to make sure the session id comes differently.
}
...
fn test_emulated_centralized_dkg_determinism() {
    ...
    // TODO: fully integrate this test.
    // it could at least test with real PP from the network DKG output that the other tests produce.
