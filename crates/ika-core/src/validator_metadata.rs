...
/// Publishing the full bundle off-chain is what lets the
/// validator-mpc_data assembler resolve all four committee key sets on a
/// v4 cluster and avoid the "0/N PVSS keys decoded" rejection during
/// network DKG and reconfig.
...
pub enum PeerBlobVerdict {
    ...
    /// (BCS error, or the inner bytes aren't a valid
    /// `ValidatorEncryptionKeysAndProofs` bundle).
    ...
    F: Fn(&[u8; 32]) -> Option<Vec<u8>>,
{
    ...
    for (authority, digest) in announcements {
        ...
        secp256r1_pvss.insert(authority, bundle.secp256r1_pvss);
        ristretto_pvss.insert(authority, bundle.ristretto_pvss);
...
mod tests {
    ...
    /// The v7 width tests lived here: a cert verifying across the flip in both
    /// directions, a straggler emitting the wrong width being excluded from the
    /// quorum, and the retry not rescuing a genuinely bad signature. All three
    /// exercised a second `AuthorityName` encoding that no supported protocol
    /// version emits any more, and the retry they tested is gone with it.
    ///
    /// The property they protected is not gone, and is now structural: there is
    /// one encoding, so a "straggler" cannot exist without running a binary
    /// this codebase does not build. What survives them is
    /// `authority_name_is_emitted_as_the_raw_consensus_key` in `ika-types`,
    /// which pins the wire bytes — the thing every signature and digest in the
    /// protocol is reconstructed from.
