...
//! need a real gRPC endpoint + faucet to publish against and read events from. So the
...
impl SuiLocalnet {
    ...
    /// gRPC endpoint answers. `sui_binary` is the path to a workspace-tag-matching `sui`.
    ...
    async fn wait_until_ready(&self, timeout: Duration) -> Result<()> {
        ...
        loop {
            ...
            // The faucet (port 9123) comes up a beat after gRPC. `init_ika_on_sui`
            ...
            let faucet_ok = grpc_ok
                && http
                    ...
                    .map(|_| true)
                    .unwrap_or(false);
            if grpc_ok && faucet_ok {
                tracing::info!(grpc = %self.grpc_url, faucet = %self.faucet_url, "sui localnet ready");
                return Ok(());
            }
            if tokio::time::Instant::now() >= deadline {
                bail!(
                    "sui localnet (grpc_ok={grpc_ok}, faucet_ok={faucet_ok}) not ready within {:?}; see {}",
                    timeout,
