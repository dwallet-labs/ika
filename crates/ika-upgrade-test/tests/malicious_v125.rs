...
//!
//! Boots a 4-validator committee on the HONEST literal v1.2.5 release
//! (deployed on both networks, protocol v5), lets it complete the genesis
...
//!
//! Expectation: the honest v1.2.5 validators must identify the faulty one as
...
//! release is not vacuous — i.e. `v125_rollout`'s zero-malicious gate would
...
//!
//! Opt-in, via `RUN_MALICIOUS_V125=1`:
//!
//! ```bash
...
//!   SUI_BIN=$(which sui) \
//!   cargo test --release -p ika-upgrade-test --test malicious_v125 -- --nocapture
//! ```
...
async fn honest_committee_marks_faulty_current_validator_malicious() {
    if std::env::var("RUN_MALICIOUS_V125").is_err() {
        eprintln!(
            "skipping: set RUN_MALICIOUS_V125=1 (needs OLD_BIN/FAULTY_BIN/NOTIFIER_BIN/IKA_BIN/SUI_BIN)"
        );
        return;
    }
    ...
    // Honest reference binary: the literal v1.2.5 ika-validator.
    ...
    let base = PathBuf::from(
        std::env::var("UPGRADE_TEST_DIR").unwrap_or_else(|_| "/tmp/ika-malicious-v125".to_string()),
    );
    ...
    Scenario::new(4, repo, sui, notifier)
        ...
        // Genesis network DKG on the all-honest v1.2.5 committee at v5.
        .start_all(honest)
        .wait_for_epoch(2)
        ...
        .run()
        .await
        .expect("honest v1.2.5 committee should reconfigure past a faulty current validator");

    tracing::info!(
        "malicious-v125 PASSED: honest v1.2.5 committee reached epoch 3 AND recorded the \
