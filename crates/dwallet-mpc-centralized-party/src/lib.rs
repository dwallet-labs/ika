...
fn advance_sign_by_protocol_with_decentralized_party_dkg_output<P>(
...
where
    P: twopc_mpc::Protocol + twopc_mpc::sign::Protocol + twopc_mpc::presign::Protocol,
{
    ...
    let centralized_party_dkg_public_output = match versioned_decentralized_dkg_output {
        VersionedDwalletDKGPublicOutput::V1(output) => {
            let versioned_output: <<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::DecentralizedPartyDKGOutput =
                bcs::from_bytes::<<<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::DecentralizedPartyTargetedDKGOutput>(output.as_slice())?
                    .into();
            versioned_output.into()
        }
        VersionedDwalletDKGPublicOutput::V2 { dkg_output, .. } => {
            bcs::from_bytes::<<<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::DecentralizedPartyDKGOutput>(dkg_output.as_slice())?.into()
        }
    };
...
fn advance_sign_by_protocol_with_centralized_party_dkg_output<P>(
    centralized_party_secret_key_share: &[u8],
    presign: &[u8],
    message: Vec<u8>,
    hash_scheme: HashScheme,
    ...
    P: twopc_mpc::Protocol + twopc_mpc::sign::Protocol + twopc_mpc::presign::Protocol,
{
    ...
    let centralized_party_dkg_public_output = match versioned_centralized_dkg_output {
        VersionedCentralizedDKGPublicOutput::V1(output) => {
            let versioned_output: <<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::CentralizedPartyDKGOutput =
                bcs::from_bytes::<<<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::CentralizedPartyTargetedDKGOutput>(output.as_slice())?.into();

            versioned_output
        }
        VersionedCentralizedDKGPublicOutput::V2(output) => {
            bcs::from_bytes::<<<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::CentralizedPartyDKGOutput>(output.as_slice())?
        }
    };
...
fn advance_sign_by_protocol<P>(
    centralized_party_secret_key_share: &[u8],
    presign: &[u8],
    message: Vec<u8>,
    hash_scheme: HashScheme,
    centralized_party_dkg_public_output: <<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::CentralizedPartyDKGOutput,
    protocol_pp: &[u8],
) -> anyhow::Result<Vec<u8>>
where
    P: twopc_mpc::Protocol + twopc_mpc::sign::Protocol + twopc_mpc::presign::Protocol,
{
    ...
    let centralized_party_secret_key_share =
        bcs::from_bytes::<<<P as twopc_mpc::presign::Protocol>::DKGProtocol as twopc_mpc::dkg::Protocol>::CentralizedPartySecretKeyShare>(&centralized_party_secret_key_share)?;

    let presign: <P as twopc_mpc::presign::Protocol>::Presign = bcs::from_bytes(presign)?;
    let centralized_party_public_input =
        <P as twopc_mpc::Protocol>::SignCentralizedPartyPublicInput::from((
            message,
            hash_scheme,
            centralized_party_dkg_public_output,
            presign,
    ...
    let round_result = <P as twopc_mpc::Protocol>::SignCentralizedParty::advance(
        (),
        &centralized_party_secret_key_share,
        &centralized_party_public_input,
        &mut OsCsRng,
